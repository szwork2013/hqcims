...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a>购物车管理</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="cart" action="${ctx}/cms/cart/" method="post">
		<form:hidden path="id"/>
	</form:form>
	<div class="breadcrumb form-search">	
		<input id="selectButton" class="btn btn-primary" type="button" value="更改客户"/>
		<label>客户名称：</label>
		<input id="consumer_name" name="consumer_name" type="text" value="${cart.consumer.name}"  readonly/>
		<input id="consumer_id" name="consumer_id" type="hidden" value="${cart.consumer.id}"/>
	</div>
	...
	<table id="contentTable" class="table table-striped table-bordered ">
		...
		<c:forEach items="${list}" var="childList" varStatus="st" >
			<tr>
				...
				<td>
				<a href="${ctx}/cms/cart/deleteDetail?id=${childList.id}" onclick="return confirmx('确认要删除该条数据吗？', this.href)">删除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
	...
	<div class="form-actions">
		...
		<input id="btnButton"  class="btn btn-primary" type="button" value="保 存修改" onclick="doSubmit()"/>
		<input id="btnButton"  class="btn btn-primary" type="button" value="订单确认" onclick="doSubmit2()"/>
		<input id="btnButton"  class="btn btn-primary" type="button" value="清空购物车" onclick="doSubmit3()"/>
		</div>
	</div>
